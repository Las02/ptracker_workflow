...
class wss_file_checker:
    ...
    def fail(
        ...
        # raise click.BadParameter(message + f" using flag '--{param}")
        raise click.BadParameter(message, param_hint=param)

    def get_info(self, value, param):
        ctx = None  # Tiny hack, to not need to change stuff everywhere
        ...
        self.logger.print(
            f"\nRead in the following sample list from '{value}' using flag '--{param}':"
        )
