...
rule avamb_default:
    ...
    output:
        ...
        dir = OUTDIR / ("sample_{sample}_" + VAMB_TYPE + "_run_1_from_bam_contig"),
    ...
    shell:
        """
        rm -rf {output.dir}
        vamb bin avamb --outdir {output.dir} --fasta {input.contig} \
        -p {threads} --bamfiles {input.bamfiles} -m 2000
        """

rulename = "avamb_rpkm_comp"
rule avamb_rpkm_comp:
    input:
    ...
    shell:
        """
        rm -rf {output.dir}
        vamb bin avamb --outdir {output.dir} --composition {input.composition} \
